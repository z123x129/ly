<template>
    <div id="i1" style="width: 100%">
        <div class="demo">
            ...
            <CheckboxGroup @on-change="chageShow" v-model="value">
        ...
        <div class="map_box">
            ...
            <div class="block_show" v-if="show">
                <el-date-picker
                        :clearable="false"
                        :editable="false"
        ...
        <videoDialog :visible.sync="videoVisible" @cancel="hideVideo" @resize="resize" @resize_1="resize_1"
                     @showvideo="showvideo" class="dialog">
    ...
    export default {
        ...
        data() {
            return {
                ...
                showBtn: true,
                newTime: '',
                img1: require('../images/jiankong.png'),
                img2: require('../images/luxian.png'),
            }
        },
        computed: {
        ...
        methods: {
            ...
            getNewTime() {
                ...
                this.newTime = now.getFullYear() + '-' + _month + '-' + _day;
            },
            ...
            getDanbing(time) {
                if (!time) {
                    ...
                    this.$https.fetchPost('/plugin/statistics/api_index/getDayCoordinate', params).then((res) => {
                        this.markerArr = res;
                        if (this.markerArr && this.value.indexOf('单兵') > -1) {
                            this.addsite(2);
                        }
                    })
                } else if (time == this.newTime) {
                    this.delsite(2, 'old');
                    if (this.markerArr && this.value.indexOf('单兵') > -1) {
                        this.addsite(2);
                    }
                    ...
                    this.delsite(2, 'new');
                    let params = {'uid': this.$store.state.user.uid, time};
                    this.$https.fetchPost('/plugin/statistics/api_index/getDayCoordinate', params).then((res) => {
                        this.historyArr = res;
                        if (this.historyArr && this.value.indexOf('单兵') > -1) {
                            this.addsite(2, 'type');
                        }
                    })
                }
            ...
            chageShow(val) {
                if (val.indexOf('单兵') > -1) {
                    if (this.markers.length > 0) {
                        ...
                        if (this.time == this.newTime || !this.time) {
                            this.addsite(2)
                        } else {
                            this.addsite(2, 'type')
                        }
            ...
            addsite(type, step) {
                if (type == 1) {
                        ...
                        for (let x in this.data[i]["children"]) {
                            for (let y in this.data[i]["children"][x]["children"]) {
                                ...
                                schoolMarker.content = '<div class="mapBox">' +
                                    ...
                                    '<img src="' + img + '">' +
                                    '<div class="text">' +
                                    '<p>联系人：' + (this.data[i]["children"][x]["children"][y].personCharge ? this.data[i]["children"][x]["children"][y].personCharge : '无') + '</p>' +
                                    '<p>联系电话：' + (this.data[i]["children"][x]["children"][y].personChargePhone ? this.data[i]["children"][x]["children"][y].personChargePhone : '无') + '</p>' +
                                    '<p>地址：' + (this.data[i]["children"][x]["children"][y].street ? this.data[i]["children"][x]["children"][y].street : '无') + '</p>' +
                    ...
                    var that = this;
                    let ArrLoad = this.markerArr;
                    if (step) {
                        ArrLoad = this.historyArr;
                    }
                    ArrLoad.map((va, key) => {
                        ...
                        contextMenu.addItem(va.cameraName, function () {

                        }, 0);
                        let htm = '<div><img style="width: 16px;height: 18px;margin-right: 8px" src="' + this.img1 + '" />查看监控<div>';
                        let htm2 = '<div><img style="width: 20px;height: 18px;margin-right: 3px" src="' + this.img2 + '" />实时轨迹<div>';
                        let htm3 = '<div><img style="width: 20px;height: 18px;margin-right: 3px" src="' + this.img2 + '" />历史轨迹<div>';
                        if (va.today) {
                            contextMenu.addItem(htm, function () {
                                that.opvideo('', va);
                            }, 0);
                        }
                        if (va.today) {
                            contextMenu.addItem(htm2, function () {
                                that.polyline(key)
                            }, 1);
                        }
                        if (!va.today) {
                            contextMenu.addItem(htm3, function () {
                                that.repolyline(key)
                            }, 1);
                        }
                        ...
                        marker.content = '';
                        // marker.content = '<div class="mapBox2"><div style="width: auto;padding-right: 20px;padding-top: 5px">'+va.cameraName+'</div></div>';
                        // marker.on('mouseover', markerClick);
                        marker.setMap(that.map);
                        if (va.today) {
                            this.markers.push(marker);
                        } else {
                            this.historys.push(marker);
                        }
                    })
            ...
            getTouxiang(key) {
                ...
                marker.setMap(this.map);
                if (va.today) {
                    this.markers.splice(key,1,marker);
                } else {
                    this.historys.push(marker);
                }
                // this.map.setZoomAndCenter(11,this.tzSite);
            ...
            delsite(type, step) {
                if (type == 1) {
                    ...
                    this.map.remove(this.historys);//删除点标记
                    this.markers = [];
                    this.historys = [];
                    // this.map.remove(this.plines);//删除轨迹
                    // this.plines =[]
                    if (this.markerArr && this.markerArr.length > 0) {
                        this.markerArr.forEach((item, index) => {
                            if (this.markerArr[index].pline) {
                                this.map.remove(this.markerArr[index].pline);
                                this.markerArr[index].start = true;
                                // this.markerArr[index] = [];
                            }
                        })
                    }
                    if (this.historyArr && this.historyArr.length > 0) {
                        this.historyArr.forEach((item, index) => {
                            if (this.historyArr[index].pline) {
                                this.map.remove(this.historyArr[index].pline);
                                // this.historyArr[index] = [];
                            }
                        })
                    }
                } else if (type == 3) {
...
<style lang="less" scoped>
    ...
    #content /deep/ .amap-menu-outer li:first-child {
        background: #F3F3EE;
        cursor: default;
    }
